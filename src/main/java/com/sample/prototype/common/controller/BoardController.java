...
import com.sample.prototype.domain.like.repository.LikeRepository;
...
public class BoardController {
	...
	@PostMapping("/admin/likes/delete")
	public String deleteLikes(@RequestParam(value = "ids", required = false) List<Long> ids) {
		logger.debug("좋아요 다중 삭제 요청: ids={}", ids);
		try {
			if (ids == null || ids.isEmpty()) {
				logger.warn("삭제할 좋아요 ID 없음");
				String encodedError = URLEncoder.encode("삭제할 좋아요를 선택하세요.", StandardCharsets.UTF_8);
				return "redirect:/admin/likes?error=" + encodedError;
			}
			likeRepository.deleteAllByIdInBatch(ids);
			logger.debug("좋아요 다중 삭제 완료: count={}", ids.size());
			String encodedMessage = URLEncoder.encode("선택한 좋아요가 삭제되었습니다.", StandardCharsets.UTF_8);
			return "redirect:/admin/likes?message=" + encodedMessage;
		} catch (Exception e) {
			logger.error("좋아요 다중 삭제 실패: error={}", e.getMessage());
			String encodedError = URLEncoder.encode("좋아요 삭제 중 오류가 발생했습니다.", StandardCharsets.UTF_8);
			return "redirect:/admin/likes?error=" + encodedError;
		}
	}

	@PostMapping("/admin/likes/{id}/delete")
	public String deleteLike(@PathVariable Long id) {
		logger.debug("좋아요 개별 삭제 요청: id={}", id);
		try {
			if (!likeRepository.existsById(id)) {
				logger.warn("삭제 대상 좋아요 없음: id={}", id);
				String encodedError = URLEncoder.encode("존재하지 않는 좋아요입니다.", StandardCharsets.UTF_8);
				return "redirect:/admin/likes?error=" + encodedError;
			}
			likeRepository.deleteById(id);
			logger.debug("좋아요 개별 삭제 완료: id={}", id);
			String encodedMessage = URLEncoder.encode("좋아요가 삭제되었습니다.", StandardCharsets.UTF_8);
			return "redirect:/admin/likes?message=" + encodedMessage;
		} catch (Exception e) {
			logger.error("좋아요 개별 삭제 실패: id={}, error={}", id, e.getMessage());
			String encodedError = URLEncoder.encode("좋아요 삭제 중 오류가 발생했습니다.", StandardCharsets.UTF_8);
			return "redirect:/admin/likes?error=" + encodedError;
		}
	}
