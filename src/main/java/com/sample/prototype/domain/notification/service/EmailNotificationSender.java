package com.sample.prototype.domain.notification.service;

import com.sample.prototype.domain.notification.model.enums.NotificationChannel;
...
public class EmailNotificationSender implements NotificationSender {
	...
	public void send(String recipient, String message) {
		...
		sendHtmlEmail(recipient, "[샘플 프로토타입] 알림", wrapSimpleMessage(message));
	}
	...
	private String extractSubject(String templateName, Map<String, Object> variables) {
		return switch (templateName) {
			case "signup-verification" -> "[샘플 프로토타입] 회원가입 이메일 인증";
			case "signup-code" -> "[샘플 프로토타입] 회원가입 이메일 인증번호";
			case "password-reset" -> "[샘플 프로토타입] 비밀번호 재설정 인증번호";
			default -> (String) variables.getOrDefault("subject", "[샘플 프로토타입] 알림");
		};
	}
